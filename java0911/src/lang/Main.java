...
public class Main {
	...
	public static void main(String[] args) {
		...
		//객체의 모든 속성 확인하기(투스트링 사용)
		//데이터를 표현하는 클래스를 만들 때는 toString()을 오버라이딩 하고 
		//확인 할 때는 아래처럼 이름만으로 확인가능.
		//toString() 메소드를 디버깅을 위한 메소드라고도 함. 
		System.out.printf("%s\n", vo2);
		
		//UserVO 인스턴스를 생성
		UserVO vo3 = new UserVO(3, "김원봉", 35);
		//vo3 가 가리키고 있는 곳의 참조를 vo4에 복사.
		UserVO vo4 = vo3; //실제 이런 문장이 있다면 잘못 쓴 문장. (참조를 복사하게된것)
		//vo4 가 자신의 num을 31로 변경.
		vo4.setNum(31);
		//vo3를 출력 - vo3의 num도 31로 변경되어 있음. 
		System.out.printf("%s\n", vo3);
